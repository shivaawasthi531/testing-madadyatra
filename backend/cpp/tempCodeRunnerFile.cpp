...
int main(int argc, char* argv[]) {
    ...
    if (!station_file.is_open()) {
        cout << "Error opening metro_stations.csv\n";
        return 1;
    }

    string line;
    getline(station_file, line); // header

    while (getline(station_file, line)) {
        stringstream ss(line);
        string metro_line, order, station;
        ...
        if (metro_line.empty() || station.empty()) continue;

        Station st;
        st.name = station;
        st.normalized = normalizeLocation(station);
    ...
    if (!info_file.is_open()) {
        cout << "Error opening metro_lines.csv\n";
        return 1;
    }

    getline(info_file, line); // header
    ...
    for (auto &p : metro_routes) {
        ...
        for (int i = 0; i < stations.size(); i++) {
            if (stations[i].normalized == from)
                from_idx = i;

            if (stations[i].normalized == to)
                to_idx = i;
        }
